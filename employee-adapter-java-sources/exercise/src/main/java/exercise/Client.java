...
public class Client {
  public static void main(String[] args) {
    ...
    List<EmployeeCSVAdapter> employeeCSVAdapters = new ArrayList<>();
    for(EmployeeCSV employeeCSV: csvRows){
        employeeCSVAdapters.add(new EmployeeCSVAdapter(employeeCSV));
    }
    
    List<EmployeeDBAdapter> employeeDBAdapters = new ArrayList<>();
    for(EmployeeDB employeeDB: dbRows){
        employeeDBAdapters.add(new EmployeeDBAdapter(employeeDB));
    }
    
    List<EmployeeLDAPAdapter> employeeLDAPAdapters = new ArrayList<>();
    for(EmployeeLDAP employeeLDAP: ldapRows){
        employeeLDAPAdapters.add(new EmployeeLDAPAdapter(employeeLDAP));
    }
    
    all.addAll(employeeDBAdapters);
    all.addAll(employeeCSVAdapters);
    all.addAll(employeeLDAPAdapters);
